...
export const TextInput = React.forwardRef<RNTextInput, NativeTextInputProps>(
    ({ style, inputStyle, className, variant = 'default', label, labelVariant = 'default', leftSlot, rightSlot, id: idProp, onFocus: onFocusProp, onBlur: onBlurProp, placeholderTextColor, ...props }, ref) => {
        ...
        if (labelVariant === 'in-field') {
            return (
                <View style={[styles.root, styles.rootInField, variantStyles, className ? (style || {}) : {}]}>
                    {leftSlot && <View style={styles.slotLeft}>{leftSlot}</View>}
                    <View style={styles.inputWrapper}>
                        {label && (
                            <Text style={[styles.labelInField, { color: inFieldLabelColor }]}>
                                {label}
                            </Text>
                        )}
                        {renderInput(true)}
