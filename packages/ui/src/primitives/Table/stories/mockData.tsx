...
export const generateData = (count: number): Invoice[] => {
  return Array.from({ length: count }).map((_, i) => ({
    ...
    method: ['credit_card', 'paypal', 'bank_transfer'][Math.floor(Math.random() * 3)] as any,
  }));
};
...
export const columns: ColumnDef<Invoice>[] = [
  ...
  },
  {
    ...
    header: 'Amount',
    size: 100,
    cell: (info: any) => <div className="text-right font-medium">{info.getValue() as string}</div>,
  },
];
...
export const EditableCell = ({ getValue, row, column, table }: any) => {
  ...
  if (column.id === 'status' || column.id === 'method') {
        ...
        ? [
            {
              ...
              onSelect: () => {
                setValue('Pending');
                table.options.meta?.updateData?.(row.index, column.id, 'Pending');
              },
            },
            {
              id: 'Paid',
            ...
            },
            {
              ...
              onSelect: () => {
                setValue('Unpaid');
                table.options.meta?.updateData?.(row.index, column.id, 'Unpaid');
              },
            },
          ]
        : [
            {
              ...
              onSelect: () => {
                ...
                table.options.meta?.updateData?.(row.index, column.id, 'Credit Card');
              },
            },
            {
              id: 'PayPal',
              ...
              onSelect: () => {
                setValue('PayPal');
                table.options.meta?.updateData?.(row.index, column.id, 'PayPal');
              },
            },
            {
              id: 'Bank Transfer',
              ...
              onSelect: () => {
                ...
                table.options.meta?.updateData?.(row.index, column.id, 'Bank Transfer');
              },
            },
          ];
    ...
    return (
      <DropdownMenu
        items={options}
        triggerLabel={(value as string) || (column.id === 'status' ? 'Status' : 'Method')}
      >
