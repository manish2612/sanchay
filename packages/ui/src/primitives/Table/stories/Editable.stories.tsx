...
export const Editable: Story = {
  ...
  render: (args) => {
    const [data, setData] = useState(() => [
      { id: "INV-1", date: "", status: "Pending", method: "Credit Card", amount: "", isPhantom: true } as any,
    ]);
    const [rowErrors, setRowErrors] = useState<Record<number, boolean>>({});

    const updateData = (rowIndex: number, columnId: string, value: unknown) => {
      setData((old: any[]) => old.map((row, index) => index === rowIndex ? { ...old[rowIndex], [columnId]: value } : row));
    };
    ...
    const onRowCommit = (rowIndex: number, columnId?: string, cellValue?: string) => {
      ...
      if (isNaN(numeric) || row.amount.trim() === "") {
        ...
        setTimeout(() => { setRowErrors((prev) => ({ ...prev, [rowIndex]: false })); }, 500);
        return "STAY"; 
      }
      if (row.isPhantom) {
        setData((old: any[]) => {
          const newData = [...old];
          const committedRow = columnId && cellValue !== undefined ? { ...newData[rowIndex], [columnId]: cellValue, isPhantom: false } : { ...newData[rowIndex], isPhantom: false };
          newData[rowIndex] = committedRow;
          newData.push({ id: `INV-${newData.length + 1}`, date: "", status: "Pending", method: "Credit Card", amount: "", isPhantom: true });
          return newData;
        });
        return "ADVANCE"; 
      }
    ...
    return (
      <div className="border border-[#222222] rounded-md h-[400px]">
        <Table.Root {...args} data={data} columns={editableColumns} className="h-full" tableOptions={{ meta: { updateData, onRowCommit, rowErrors, phantomRowConfig: { isPhantom: (row: any) => row.original.isPhantom, actionText: "Add New Invoice" } } }}>
          <Table.Header>
            {({ table }) => (
              <>
            ...
            {(row, isFocused) => (
              <Table.Row key={row.id} data-state={row.getIsSelected() ? "selected" : undefined} data-focused={isFocused}>
                {row.getVisibleCells().map((cell) => (
                  <Table.Cell key={cell.id} style={{ width: cell.column.getSize(), flex: `${cell.column.getSize()} 0 auto` }} className="py-0">
