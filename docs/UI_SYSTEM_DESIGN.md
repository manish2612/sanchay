...
Instead of the component deciding _how_ to navigate, it just signals _that_ an action occurred. The parent component (living in the specific App) injects the navigation logic.
...
export const UserCard = ({ name, onViewProfile }: UserCardProps) => (
  ...
  <div onClick={() => onViewProfile('123')}>View Profile</div>
);
```
